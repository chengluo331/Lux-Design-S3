...
def get_obs(players, my_obs, params):
    ...
    # init obs
    units_position = np.zeros((num_teams, width, height), dtype=np.float32)
    ...
    opp_n = players.opp_n
    # units
    u_weight = 1./max_units
    energy_range = max_energy - min_energy
    for u in range(max_units):
        for t in (me_n, opp_n):
            if u_mask[t][u]:
                units_position[t, *pos[t][u]] += u_weight
    ...
    map_features_energy = my_obs['map_features']['energy'].astype(np.float32)
...
class Observation:
    ...
    def get_space(self):
        ...
        return Dict({
            "units_position": Box(low=0., high=1., shape=(num_teams, width, height), dtype=np.float32),
            "units_energy": Box(low=0., high=1., shape=(num_teams, width, height), dtype=np.float32),
            ...
            "map_features_energy": Box(low=0., high=1., shape=(width, height), dtype=np.float32),
    ...
    def get_obs(self):
        my_obs = self.last_global_obs[self._players.me]
        return get_obs(self._players, my_obs, self._env_params)
